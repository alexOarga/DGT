...
from graphgps.loader.dataset.custom_dataset import CustomDataset
...
def load_dataset_master(format, name, dataset_dir):
    ...
    """
    print("LOADER PARAMS: ", format, name, dataset_dir)
    if name == 'custom':
        dataset = preformat_custom_dataset(dataset_dir, feature_set=format)
    ...
    if pe_enabled_list:
        ...
        SKIP_COMP = "dataset.pkl"
        '''chek if file exists in current path'''
        import os
        if os.path.isfile(SKIP_COMP):
            logging.info(f"** Using precomputed dataset.pkl")
            dataset = torch.load(SKIP_COMP)
        else:
            logging.info(f"** Generating new dataset.pkl")
    ...
    # TRICK...
    #dataset.data['train_graph_index'] = dataset.data['train_graph_index'][250:350]
    #dataset.slices['train_graph_index'][1] = 100
    #print(dataset.data['train_graph_index'])
...
def preformat_custom_dataset(dataset_dir, feature_set):
    """
    """
    ...
    logging.info(f'Computing "{feature_set}" node features for CustomDataset.')
    pre_transform_in_memory(dataset, tf)
