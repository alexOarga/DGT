...
def train_epoch(logger, loader, model, optimizer, scheduler, batch_accumulation):
    ...
    for batch in loader:
        ...
        batch.split = 'train'
        ############################################################################################################
        batch.EigVals = torch.reshape(batch.EigVals, (-1, cfg.posenc_MagLapPE.eigen.max_freqs))
        transform = T.Compose([T.ToUndirected(), T.AddSelfLoops(attr="edge_attr")])
        ...
        if cfg.dataset.name == 'ogbg-code2':
            ...
            pred_int = _get_pred_int(_pred)
            true_int = _true.squeeze(-1)
            if iter == 1:
                print(confusion_matrix(true_int, pred_int))
            ############################################################
            '''
            if batch.x.shape[0] == 170 or batch.x.shape[0] < 80:
                pred_score = _pred
                pred_int = _get_pred_int(pred_score)
                true_int = _true.squeeze(-1)
                acc = accuracy_score(true_int, pred_int)
                pre = precision_score(true_int, pred_int)
                print("train[0]=", iter, round(float(acc), cfg.round), round(float(pre), cfg.round))
                print(pred_int)
            '''
        loss.backward()
        ...
        try:
            ...
            if cfg.dataset.name == 'ogbg-code2':
                ...
                if cfg.dataset.task_type == 'regression':
                    ...
                    pred_int = _get_pred_int(pred_score)
                    true_int = _true.squeeze(-1)
                    assert pred_int.shape == true_int.shape
                    if iter == 0:
                        print(confusion_matrix(true_int, pred_int))
                    iter += 1
                    if _pred.ndim > 1:  # multiclass classification
                        acc = accuracy_score(true_int, pred_int)
                        pre = precision_score(true_int, pred_int, average='macro')
                    else:
                        acc = accuracy_score(true_int, pred_int)
                        pre = precision_score(true_int, pred_int)
                #print(iter, round(float(acc), cfg.round), round(float(pre), cfg.round), batch.x.shape)
            ...
            '''
            if iter == 0 and split == 'test':
                for li in range(pred.shape[0]):
                    print("pred[", li, "]=", pred[li], "true[", li, "]=", true[li])
            '''

        except Exception as err:
            print("Skipped: ", str(err))
            raise err
...
def custom_train(loggers, loaders, model, optimizer, scheduler):
    ...
    for cur_epoch in range(start_epoch, cfg.optim.max_epoch):
        ...
        if is_eval_epoch(cur_epoch) and len(val_perf) > 0:
