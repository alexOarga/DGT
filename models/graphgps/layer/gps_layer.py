...
class GPSLayer(nn.Module):
    ...
    def __init__(self, dim_h,
        ...
        if local_gnn_type == 'None':
            ...
            self.local_model_2 = GatedGCNLayer(dim_h, dim_h,
                                             dropout=dropout,
                                             residual=True,
                                             act=act,
                                             equivstable_pe=equivstable_pe)
        ...
        self.act_fn_ff = register.act_dict['gelu']()
        if self.layer_norm:
        ...
        '''
        final_nn = nn.Sequential(Linear_pyg(dim_h * 2, dim_h),
                               self.activation(),
                               Linear_pyg(dim_h, dim_h))
        if self.equivstable_pe:  # Use specialised GINE layer for EquivStableLapPE.
            self.final_model = GINEConvESLapPE(final_nn)
        else:
            self.final_model = pygnn.GINEConv(final_nn, edge_dim=dim_h)
        self.final_linear = Linear_pyg(dim_h * 2, dim_h)
        '''
    ...
    def forward(self, batch):
        ...
        if self.local_model is not None:
            ...
            if self.local_gnn_type == 'CustomGatedGCN':
                ...
                '''
                local_out_2 = self.local_model_2(Batch(batch=local_out_1,
                                                   x=local_out_1.x,
                                                   edge_index=local_out_1.edge_index,
                                                   edge_attr=local_out_1.edge_attr,
                                                   pe_EquivStableLapPE=es_data))
                '''
                # GatedGCN does residual connection and dropout internally.
                h_local = local_out_1.x
                batch.edge_attr = local_out_1.edge_attr
            else:
                if self.equivstable_pe:
        ...
        '''
        out = self.final_model(torch.concat((h, batch.x), dim=-1), batch.edge_index, batch.edge_attr)
        h = self.final_linear(torch.concat((h, out), dim=-1))
        '''
